...
const Layout = ({ pageProps, Component, pageLoading, ready, emotionCache, displayHeader, authenticated, cssVariable, cookieNotice }: any) => {
  return (
    <>
      <Head>
        ...
        <ThemeProvider theme={theme}>
          ...
          <SnackbarProvider maxSnack={2} preventDuplicate anchorOrigin={{ horizontal: "right", vertical: "top" }}>
            <main className={styles.layout} style={cssVariable}>
              <HeaderContainer authenticated={authenticated} displayHeader={displayHeader} relativeHeader={null} titleOnly={null} />
              <div>
                {ready && pageLoading && <BuilderLoading />}
                {ready && !pageLoading && (
                  <main>
                    <Component {...pageProps} />
